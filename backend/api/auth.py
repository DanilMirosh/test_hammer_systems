...
class AuthService:
    ...
    def get_user_from_token(self, *, token: str):
        ...
        try:
            decoded_token = jwt.decode(token, str(settings.SECRET_KEY), audience=self.JWT_AUDIENCE,
                                       algorithms=[self.JWT_ALGORITHM])
            payload = User(id=decoded_token["id"], phone_number=decoded_token["phone_number"],
