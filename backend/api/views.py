...
def refresh_token(request):
    ...
    serialized = TokensSerializer(
        data={"access_token": request.COOKIES["access_token"], "refresh_token": request.COOKIES["refresh_token"]})
    if serialized.is_valid():
        refreshed = refresh_token_pair(serialized)
        serialized = TokensSerializer(
            data={"refresh_token": refreshed.refresh_token, "access_token": refreshed.access_token})
        if serialized.is_valid():
            return Response(serialized.data)
